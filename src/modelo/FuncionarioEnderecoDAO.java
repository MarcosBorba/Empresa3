...
public class FuncionarioEnderecoDAO {
	public static int create(Enderecos o) {
		...
		try {
			...
			PreparedStatement stm = conn.prepareStatement(
					"INSERT INTO funcionariosenderecos(fk_funcionario, logradouro, bairro, cidade, estado, pais, cep) VALUES (?, ?, ?, ?, ?, ?, ?);",
	...
	public static Enderecos retreave(int pk) {
		try {
			...
			PreparedStatement stm = conn
					.prepareStatement("select * from funcionariosenderecos where pk_endereco = ?");
			...
			return new Enderecos(pk, rs.getInt("fk_funcionario"), rs.getString("logradouro"), rs.getString("bairro"),
	...
	public static ArrayList<Enderecos> retreaveAll(int fk) {
		try {
			...
			Connection conn = BancoDados.createConnection();
			PreparedStatement stm = conn
					.prepareStatement("select * from funcionariosenderecos where fk_funcionario = ?");
			stm.setInt(1, fk);
	...
	public static void update(Enderecos o) {
		...
		try {
			...
			Connection conn = BancoDados.createConnection();
			PreparedStatement stm = conn.prepareStatement(
					"UPDATE funcionariosenderecos SET fk_funcionario=?, logradouro=?, bairro=?, cidade=?, estado=?, pais=?, cep=? WHERE pk_endereco = ?");
	...
	public static void delete(Enderecos o) {
		...
		try {
			...
			Connection conn = BancoDados.createConnection();
			PreparedStatement stm = conn.prepareStatement("DELETE FROM funcionariosenderecos WHERE pk_endereco = ?");
